...
export default function MissionScreen() {
	...
	const [missions, setJob] = useState<Job[]>([]);
	...
	const renderMissionCard = ({ item }: { item: Job }) => (
		<DefaultCard style={[styles.card, { backgroundColor: colors.background || "#ffffff" }]}>
			<View style={styles.cardHeader}>
				...
				<View style={styles.missionContent}>
					...
					{item.description && (
						<Text
							style={[styles.description, { color: colors.secondary || "#666" }]}
							numberOfLines={1}
						>
							{item.description}
						</Text>
					...
					<View style={styles.infoRow}>
						...
						<Text style={[styles.infoText, { color: colors.secondary || "#666" }]} numberOfLines={1}>
							{item.address}
						</Text>
					</View>
